import { CELLS, PROOFS, BEHAVIOR_CELLS, EVENT_CELLS } from "@/lib/data/mock";
  ...
  const objects = [
    {
      ...
      color: "#eab308",
      label: "Precursor",
      count: CELLS.filter(c => c.cell_type === "precursor").length,
      description: "The early-warning signal — a condition that precedes harm if uncontrolled.",
    },
    {
      type: "control",
      color: "#22c55e",
      label: "Control",
      count: provenCount,
      description: "A safeguard that is verified and active — the system is working.",
    },
    {
      type: "failure",
      color: "#ef4444",
      label: "Failure",
      count: failureCount,
      description: "A control that is missing, expired, or bypassed — the gap in the defence.",
    },
    {
      type: "behavior",
      color: "#a855f7",
      label: "Behavior",
      count: BEHAVIOR_CELLS.length,
      description: "A recurring human or organisational pattern that amplifies risk.",
    },
    {
      type: "event",
      color: "#3b82f6",
      label: "Event",
      count: EVENT_CELLS.length,
      description: "An outcome: incident, near-miss, claim, or audit finding.",
    },
    {
      type: "learning",
      color: "#14b8a6",
      label: "Learning",
      count: learningCount,
      description: "An insight that folds back into the model to prevent recurrence.",
    },
  ];
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="iq-scroll flex-1 overflow-y-auto p-6">
            ...
            <Card key={obj.type}>
              <div className="p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: obj.color }}>
                      {obj.label}
                    </p>
        ...
        {/* ARC continuous loop diagram + lists */}
          ...
          <Card>
            ...
            <div className="flex justify-center px-4 pb-4">
              <svg viewBox="0 0 440 390" width="320" height="280" xmlns="http://www.w3.org/2000/svg">
                ...
                />
                {svgNodes.map((node, i) => {
                  const next = svgNodes[(i + 1) % svgNodes.length];
                  ...
                  return (
                    <g key={`arrow-${i}`}>
                      <circle cx={mx + (dx / len) * 0} cy={my + (dy / len) * 0} r="2" fill={node.color} opacity="0.7" />
                    </g>
                  );
                })}
                {svgNodes.map(node => (
                  <g key={node.key}>
                    ...
                    >
                      {objects.find(o => o.type === node.key)?.count ?? 0}
                    </text>
                  </g>
                ))}
                <text x="220" y="192" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="system-ui,sans-serif" fontWeight="600">
                  ARC
                </text>
                <text x="220" y="203" textAnchor="middle" fill="#94a3b8" fontSize="8" fontFamily="system-ui,sans-serif">
                  living system
                </text>
              </svg>
            </div>
            <p className="px-4 pb-4 text-center text-xs text-slate-500">
              Each object class connects causally — the loop is continuous.
            </p>
          </Card>

          {/* Event + Behavior cells */}
                ...
                {EVENT_CELLS.map(evt => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        background: `${SEVERITY_COLORS[evt.severity] ?? "#94a3b8"}18`,
                        color: SEVERITY_COLORS[evt.severity] ?? "#94a3b8",
                      }}
                    >
                      {evt.severity}
                    </span>
            ...
            <Card>
              <CardHeader
                ...
                right={
                  ...
                  >
                    {BEHAVIOR_CELLS.length}
                  </span>
                }
              />
              <div className="space-y-2 px-4 pb-4">
                {BEHAVIOR_CELLS.map(beh => (
                  <div key={beh.id} className="rounded-lg bg-slate-50 px-3 py-2.5">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="shrink-0 rounded bg-purple-100 px-1.5 py-0.5 text-[11px] font-semibold text-purple-700">
                        {beh.occurrences}× seen
                      </span>
                    </div>
