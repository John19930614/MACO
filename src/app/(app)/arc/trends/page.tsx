import { CELLS, PROOFS, EDGES } from "@/lib/data/mock";
    ...
    <div className="space-y-1.5">
      {entries.map(({ key, count }) => (
        <div key={key} className="flex items-center gap-3">
          <span className="w-28 shrink-0 truncate text-[11px] text-slate-500 capitalize">
            {key.replace(/_/g, " ")}
          </span>
...
export default function TrendsPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <PageHeader
        title="Trends"
        subtitle="Patterns across every Safety Cell — the system-wide view."
      />
        ...
        <div className="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Card>
            ...
            <div className="px-4 pb-4">
              <BarList entries={controlGapDist} color="#f97316" total={total} />
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="px-4 pb-4">
              <BarList entries={exposureTypeDist} color="#3b82f6" total={total} />
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="px-4 pb-4">
              <BarList entries={energySourceDist} color="#a855f7" total={total} />
            </div>
          </Card>
              ...
              <div className="space-y-1.5">
                {statusDist.map(({ key, count }) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-3">
                      <span
                        className="w-24 shrink-0 text-[11px] font-semibold capitalize"
                        style={{ color }}
                      >
                        {key.replace(/_/g, " ")}
                      </span>
              ...
              <div key={locId} className="flex items-center gap-4">
                ...
                <div className="flex-1 h-2 rounded-full bg-slate-100">
                  <div
                    ...
                    style={{ width: `${Math.round((count / maxLocCount) * 100)}%` }}
                  />
                </div>
                <span className="w-6 shrink-0 text-right text-[11px] font-semibold text-slate-600">{count}</span>
              </div>
