import { CELLS, PROOFS, BEHAVIOR_CELLS, EVENT_CELLS, ACTIONS } from "@/lib/data/mock";
...
export default function RDashPage() {
  ...
  const behaviorCount = BEHAVIOR_CELLS.length;
  const eventCount    = EVENT_CELLS.length;
  ...
  const recentEvents = [...EVENT_CELLS]
  ...
  return (
    <div className="flex h-full flex-col">
      <PageHeader
        ...
        subtitle="System-wide risk intelligence at a glance."
      />
      <div className="iq-scroll flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <Stat label="Behavior Clusters"  value={behaviorCount}                       accent="#a855f7" strip="#a855f7" />
          <Stat label="Events Logged"      value={eventCount}                          accent="#3b82f6" strip="#3b82f6" />
        </div>

        {/* Top 5 cells + distribution charts */}
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 rounded-full bg-slate-200">
                        <div
                          ...
                          style={{ width: `${cell.risk_score}%`, background: sevColor }}
                        />
                      </div>
                  ...
                  <div key={gap} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 rounded-full bg-slate-100">
                      <div
                        ...
                        style={{ width: `${Math.round((count / maxGap) * 100)}%` }}
                      />
                    </div>
                  ...
                  return (
                    <div key={sev} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-2 rounded-full bg-slate-100">
                        <div
                          className="h-full rounded-full"
                          style={{ width: `${Math.round((count / maxSev) * 100)}%`, background: SEVERITY_COLORS[sev] }}
                        />
                      </div>
                      <span className="w-5 shrink-0 text-right text-[11px] font-semibold text-slate-600">{count}</span>
                    </div>
          ...
          {recentEvents.length === 0 ? (
            ...
            <div className="divide-y divide-slate-50">
              {recentEvents.map(evt => {
                ...
                const date = new Date(evt.occurred_at).toLocaleDateString("en-AU", {
                ...
                return (
                  <div key={evt.id} className="flex items-center justify-between gap-4 px-4 py-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <span
                        className="shrink-0 rounded px-1.5 py-0.5 text-[11px] font-semibold capitalize"
                        style={{ background: `${sevColor}18`, color: sevColor }}
                      >
                        {evt.severity}
                      </span>
