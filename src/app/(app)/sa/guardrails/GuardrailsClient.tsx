...
import { upsertGuardrail } from "@/lib/actions/sa";
...
// ── Default catalog ─────────────────────────────────────────────────────────────
// These are the platform's guardrail definitions. Persisted rows (sa_guardrails,
// keyed by `key`) are merged over these for enabled/threshold. Saving upserts
// every visible rule, so the first Save materialises the catalog in the DB.

const CATALOG: GuardrailRule[] = [
  // AI Behavior
  {
...
// Merge persisted DB rows over the static catalog (matched by key).
function mergeRules(catalog: GuardrailRule[], rows: Guardrail[]): GuardrailRule[] {
  const byKey = new Map(rows.map(r => [r.key, r]));
  return catalog.map(rule => {
    const row = byKey.get(rule.id);
    if (!row) return rule;
    return {
      ...rule,
      enabled: row.enabled,
      threshold: rule.threshold && row.threshold !== null
        ? { ...rule.threshold, value: row.threshold }
        : rule.threshold,
    };
  });
}
...
export default function GuardrailsClient({ initialGuardrails }: { initialGuardrails: Guardrail[] }) {
  const router = useRouter();
  const initialRules = useMemo(() => mergeRules(CATALOG, initialGuardrails), [initialGuardrails]);
  ...
  async function handleSave() {
    setSaving(true);
    setError("");
    // Upsert one row per rule, keyed by `key`.
    for (const rule of rules) {
      const fd = new FormData();
      fd.set("key", rule.id);
      fd.set("label", rule.label);
      fd.set("enabled", rule.enabled ? "true" : "false");
      fd.set("threshold", rule.threshold ? String(rule.threshold.value) : "");
      fd.set("notes", rule.description);
      const res = await upsertGuardrail(null, fd);
      if (!res.ok) {
        setSaving(false);
        setError(res.error ?? `Failed to save "${rule.label}".`);
        return;
      }
    }
    setSaving(false);
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      {error && (
        <div className="shrink-0 border-b border-red-100 bg-red-50 px-6 py-2.5 text-xs text-red-700">{error}</div>
      )}
