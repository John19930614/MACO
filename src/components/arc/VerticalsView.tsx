...
import type { Site, SafetyCell, HslReading, VelaInsight, ControlProof, SafetyAction } from "@/lib/types";
...
): { liveStats: VerticalStats[]; availableVerticals: Vertical[] } {
  const liveSlugs = new Set(sites.map(s => s.vertical).filter(Boolean));

  const liveStats: VerticalStats[] = sites
    .filter(site => site.vertical)
    .map(site => {
      ...
      return {
        ...
        topCells,
        hslReadings: hslReadings.filter(r => r.site_id === site.id),
        velaPatterns: velaInsights.filter(v =>
          (v.applies_to as string[]).includes(site.vertical as string)
        ),
        topGaps,
      } satisfies VerticalStats;
    })
    .filter((s): s is VerticalStats => s !== null);
...
export function VerticalsPage({
  ...
  const sites        = (!MOCK_MODE && propSites)   ? propSites   : (ARC_SITES as unknown as Site[]);
  const cells        = (!MOCK_MODE && propCells)   ? propCells   : CELLS;
  const proofs       = (!MOCK_MODE && propProofs)  ? propProofs  : PROOFS;
  const actions      = (!MOCK_MODE && propActions) ? propActions : ACTIONS;
  const hslReadings  = (!MOCK_MODE && propHsl)     ? propHsl     : HSL_READINGS;
  const velaInsights = (!MOCK_MODE && propVela)    ? propVela    : VELA_INSIGHTS;

  const { liveStats, availableVerticals } = useMemo(
    () => buildStats(sites, cells, proofs, actions, hslReadings, velaInsights),
    [sites, cells, proofs, actions, hslReadings, velaInsights],
  );
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="iq-scroll flex-1 overflow-y-auto p-6">
        ...
        <section className="mb-4">
          ...
          {liveStats.length === 0 && (
            <p className="text-sm text-slate-400">No sites with a vertical configured yet. Add a site with a GUS vertical during onboarding.</p>
          )}
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    ...
                    <div className="rounded-lg bg-slate-50 px-2 py-1.5 text-center">
                      <div className="text-xs font-bold" style={{ color: s.criticalCount > 0 ? "#ef4444" : s.highCount > 0 ? "#f97316" : "#22c55e" }}>
          ...
          <div className="mb-6 overflow-hidden rounded-xl border border-blue-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 bg-blue-50 px-5 py-4">
              <div className="flex flex-wrap items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-base font-bold text-slate-900">{selected.vertical.name} GUS Engine</div>
                  <div className="mt-0.5 text-xs text-slate-500">
                    {selected.site.name}
                    {selected.site.state && ` · ${selected.site.state}`}
                    {selected.site.country && `, ${selected.site.country}`}
                    {(selected.site as Site & { headcount?: number }).headcount
                      ? ` · ${(selected.site as Site & { headcount?: number }).headcount} headcount`
                      : ""}
                  </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(VERTICAL_FOCUS[selected.vertical.slug]?.regs ?? []).map(r => (
                    <span key={r} className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[11px] text-slate-500">{r}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 divide-y divide-slate-100 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
              <div className="p-5">
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-500">{selected.totalCells} total</span>
                  <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[11px] text-blue-600">{selected.openCells} open</span>
                </div>
                  ...
                  {selected.topCells.map(c => (
                    <div key={c.id} className="flex items-start gap-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
                      ...
                      <div className="shrink-0 text-right">
                        <span className={`rounded px-1.5 py-0.5 text-[11px] font-semibold ${SEV_COLORS[c.severity]}`}>{c.severity}</span>
                        <div className="mt-0.5 text-[11px] font-bold text-slate-500">{c.risk_score}</div>
                      </div>
                    </div>
                  ))}
                  {selected.topCells.length === 0 && <p className="text-xs text-slate-400">No cells recorded.</p>}
                </div>
                        ...
                        return (
                          <div key={gap} className="flex items-center gap-2">
                            ...
                            <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-100">
                              <div className="h-full rounded-full bg-orange-400" style={{ width: `${(count / max) * 100}%` }} />
                            </div>
              ...
              <div className="space-y-5 p-5">
                <div>
                  ...
                  <div className="space-y-1.5">
                    {selected.hslReadings.map(r => {
                      ...
                      return (
                        <div key={r.id} className="flex items-center gap-2">
                          <span className="w-32 truncate text-[11px] text-slate-500">{HSL_LABELS[r.dimension] ?? r.dimension}</span>
                          <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-100">
                            <div className={`h-full rounded-full ${HSL_BAR_COLOR[level]}`} style={{ width: `${r.value}%` }} />
                          </div>
                    ...
                    {selected.hslReadings.length === 0 && <p className="text-[11px] text-slate-400">No HSL readings recorded.</p>}
                  </div>
                </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-3">
                        <div className="h-3 flex-1 overflow-hidden rounded-full bg-slate-100">
                          <div className="h-full rounded-full bg-emerald-400" style={{ width: `${((selected.proofTotal - selected.proofGapCount) / selected.proofTotal) * 100}%` }} />
                        </div>
                        <span className="text-xs font-semibold text-slate-700">{selected.proofTotal - selected.proofGapCount}/{selected.proofTotal} proven</span>
                      </div>
                      {selected.proofGapCount > 0 && (
                        <p className="text-[11px] text-red-600">{selected.proofGapCount} control{selected.proofGapCount > 1 ? "s" : ""} missing or expired</p>
                      )}
                    </div>
                  ) : (
                ...
                {selected.velaPatterns.length > 0 && (
                  <div>
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <span className="rounded bg-violet-100 px-1.5 py-0.5 text-[11px] font-bold text-violet-700">VELA</span>
                    </div>
                    <div className="space-y-2">
                      {selected.velaPatterns.map(p => (
                        <div key={p.id} className="rounded-lg border border-violet-100 bg-violet-50 p-2.5">
                          <div className="flex items-center gap-2">
                            <span className="text-[11px] font-semibold text-violet-800">{p.pattern.replace(/-/g, " ")}</span>
                            <span className="ml-auto text-[11px] font-medium text-violet-600">{Math.round(p.confidence * 100)}% confidence</span>
                          </div>
                          <p className="mt-1 text-[11px] leading-relaxed text-violet-700">{p.summary}</p>
                          <div className="mt-1.5 flex flex-wrap gap-1">
                            {p.regulatory_refs.map(r => (
                              <span key={r} className="rounded border border-violet-200 bg-white px-1.5 py-0.5 text-[10px] text-violet-500">{r}</span>
                            ))}
                          </div>
                        </div>
                      ))}
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="ml-auto text-slate-400">P-CLSS last run: today</span>
                  </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6">
            {availableVerticals.map(v => (
              <div key={v.slug} className="rounded-xl border border-slate-200 bg-white p-3 text-center transition hover:bg-slate-50">
                <div className="mb-1 text-xl">{ICON_MAP[v.icon] ?? "🏭"}</div>
                <div className="text-xs font-medium leading-tight text-slate-500">{v.name}</div>
                <div className="mt-1.5 inline-block rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-400">Available</div>
              </div>
            ))}
          </div>
        </section>
