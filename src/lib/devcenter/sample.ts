...
export const SAMPLE_TASKS: DevTask[] = [
  { id: "task-1", title: "Add a CSV export button to the Incidents page", description: "Let safety managers download the incident list as a spreadsheet.", target_area: "Incidents page", priority: "high", status: "in_progress", risk_level: "medium", metadata: {}, created_by: "you", created_at: ago(180), updated_at: ago(12) },
  { id: "task-2", title: "Fix the login redirect for invited users", description: "Invited users land on the wrong page after setting their password.", target_area: "Login / auth", priority: "urgent", status: "awaiting_approval", risk_level: "high", metadata: {}, created_by: "you", created_at: ago(420), updated_at: ago(8) },
  { id: "task-3", title: "Make the Chemicals list page load faster", description: "The list is slow when a site has thousands of chemicals.", target_area: "Chemicals page", priority: "medium", status: "in_review", risk_level: "medium", metadata: {}, created_by: "you", created_at: ago(1500), updated_at: ago(95) },
  { id: "task-4", title: "Add a dark theme to the Reports screens", description: "Match the dark theme used elsewhere in the platform.", target_area: "Reports", priority: "low", status: "queued", risk_level: "low", metadata: {}, created_by: "you", created_at: ago(60), updated_at: ago(60) },
  { id: "task-5", title: "Refresh the empty-state messages on the dashboard", description: "Friendlier wording when there is no data yet.", target_area: "Dashboard", priority: "low", status: "done", risk_level: "low", metadata: {}, created_by: "you", created_at: ago(4320), updated_at: ago(2880) },
  { id: "task-6", title: "Find out why last night's data import failed", description: "The nightly import job stopped halfway.", target_area: "Data import", priority: "high", status: "failed", risk_level: "high", metadata: {}, created_by: "you", created_at: ago(700), updated_at: ago(540) },
];
...
export function dashboardMetrics(): DashboardMetric[] {
  ...
  const openTasks = SAMPLE_TASKS.filter((t) => !["done", "rejected", "cancelled"].includes(t.status)).length;
