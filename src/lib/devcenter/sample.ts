...
export function dashboardMetrics(live?: Partial<Record<string, number>>): DashboardMetric[] {
  ...
  return [
    ...
    { key: "security_warnings", label: "Security blockers", value: securityWarnings, hint: "Critical findings blocking release", tone: securityWarnings ? "danger" : "success", href: buildTaskFilterHref("security_warnings") },
    { key: "xp_failures", label: "Experience issues", value: xpFailures, hint: "Ease-of-use problems found", tone: xpFailures ? "warn" : "success", href: buildTaskFilterHref("xp_failures") },
    { key: "draft_plans", label: "Draft artifacts", value: draftPlans, hint: "Code drafts awaiting your review", tone: draftPlans ? "info" : "neutral", href: buildTaskFilterHref("draft_plans") },
    { key: "active_prs", label: "Open pull requests", value: activePRs, hint: "PRs open on GitHub", tone: "info", href: buildTaskFilterHref("active_prs") },
...
/**
 * Sample-mode task-id sets behind each of the 4 stat-card filter keys, using
 * the exact same predicates `dashboardMetrics()` uses above (lines 201-204),
 * just mapped to task ids instead of counts.
 */
export function sampleTaskFilterBucketIds(): Record<TaskStatusFilterKey, string[]> {
  const dedupe = (ids: (string | null)[]) => [...new Set(ids.filter((id): id is string => !!id))];
  return {
    draft_plans: dedupe(SAMPLE_FILE_PLANS.filter((p) => p.status === "planned" || p.status === "needs_approval").map((p) => p.task_id)),
    active_prs: dedupe(SAMPLE_DEPLOYMENTS.filter((d) => d.status === "pr_open").map((d) => d.task_id)),
    security_warnings: dedupe(SAMPLE_SECURITY_REVIEWS.filter((s) => s.verdict === "fail" || s.verdict === "needs_changes").map((s) => s.task_id)),
    xp_failures: dedupe(SAMPLE_EXPERIENCE_REVIEWS.filter((e) => e.verdict === "changes_requested" || e.verdict === "rejected").map((e) => e.task_id)),
  };
}
