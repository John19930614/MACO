...
function incidentToInput(row: Record<string, unknown>): CspValidationInput {
  ...
  return {
    ...
    fields: {
      ...
      contractor_or_company: str(row.contractor_or_company),
      ...
      final_corrective_action: str(row.final_corrective_action),
      supervisor_review: str(row.supervisor_review),
      safety_review: str(row.safety_review),
      human_recordability_decision: str(row.recordability_decision),
    },
    evidence_count: 0,
    signals,
  };
